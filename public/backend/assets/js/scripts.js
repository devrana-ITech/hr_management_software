(function($) {
	...
	if ($(".select-customer select").length) {
		$('.select-customer select').each(function (i, obj) {
			...
			$(this).select2({
				allowClear: true,
				placeholder: typeof $(this).data('placeholder') !== "undefined" ? $(this).data('placeholder') : $lang_choose_customer,
				ajax: {
					url: _url + '/ajax/get_table_data?table=' + $(this).data('table') + '&value=' + $(this).data('value') + '&display=' + $(this).data('display') + display2 + divider + '&where=' + $(this).data('where'),
					processResults: function (data) {
						return {
				...
				target_select = $(this);
				if(typeof $(this).data('href') !== "undefined"){
					$(".select2-results:not(:has(a))").append('<p class="border-top m-0 p-2"><a class="ajax-modal" href="'+ $(this).data('href') +'" data-title="'+ $(this).data('title') +'" data-reload="false"><i class="fas fa-plus-circle mr-1"></i>'+ $lang_add_new +'</a></p>');
				}
			});
