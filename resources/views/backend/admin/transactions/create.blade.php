	...
	<div class="{{ $alert_col }}">
		<div class="card">
		    <div class="card-header d-flex align-items-center">
				<span class="panel-title">{{ _lang('Record Journal Entry') }}</span>
			</div>
			<div class="card-body">
                <form class="validate" action="{{ route('transactions.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                    ...
                                    <tr>
                                        <td class="pl-3">
                                            <select class="form-control" name="entries[{{ $index }}][account_id]" required>
                                                ...
                                                @foreach($accounts as $account)
                                                    <option value="{{ $account->id }}" {{ old("entries.$index.account_id") == $account->id ? 'selected' : '' }}>
                                        ...
                                        <td>
                                            <select class="form-control" name="entries[{{ $index }}][type]" required>
                                                <option value="debit" {{ old("entries.$index.type") == 'debit' ? 'selected' : '' }}>{{ _lang('Debit') }}</option>
                                                <option value="credit" {{ old("entries.$index.type") == 'credit' ? 'selected' : '' }}>{{ _lang('Credit') }}</option>
                                            </select>
                                        </td>
                                        <td>
                                            <input type="text" class="form-control float-field @error('entries.'.$loop->index.'.amount') is-invalid @enderror" name="entries[{{ $index }}][amount]" value="{{ old("entries.$index.amount") }}" required>
                                        </td>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <input type="file" class="dropify" id="attachment" name="attachment">
                            </div>
                        </div>
                    </div>
